...
def main():
    ...
    parser.add_argument("-n", "--colnames", help="column names to use if no names in first line")
    parser.add_argument("-kf", "--keyfile", help="file with api keys")
    ...
    if not isinstance(args.colnames, type(None)):
        names = args.colnames.split()
    else:
        names=None
    if args.inputfile.endswith('.tsv'):
        inputfile_data = pd.read_csv(args.inputfile, sep='\t', names=names)
        print('tab seperated file, using \\t delimiter')
    elif args.inputfile.endswith('.csv'):
        inputfile_data = pd.read_csv(args.inputfile, names=names)
    elif args.inputfile.endswith('.tsv.gz'):
    ...
    while True:
        try:
            with open(output_file, 'a') as outfile:
                for go in range(i):
                    print('currently getting {} - {} ({}/{})'.format(start, end, go, i))
